...
try:
    ...
    # Define dummy decorators if LangFuse not available
...
# ==================== RAG CHAT ENDPOINT WITH LANGFUSE ====================

@app.post("/api/chat")
@observe(name="rag_chat_endpoint", as_type="generation")
async def chat(request: Request):
    """Complete RAG Chat endpoint with retrieval and generation + LangFuse tracing"""
    try:
        ...
        logger.info(f"⚙️ Config: model={model}, temp={temperature}, max_tokens={max_tokens}")
        logger.info(f"⚙️ Retrieval: top_k={top_k}, threshold={score_threshold}")
        ...
        # Step 2: Retrieve context (auto-traced via @observe in retriever.py)
        ...
        # Step 3: Generate response (auto-traced via @observe in generator.py)
        ...
        for idx, chunk in enumerate(chunks_to_return):
            ...
            if not file_name or not text or score is None:
                logger.warning(f"⚠️ Skipping chunk {idx + 1}: missing required fields")
                continue
