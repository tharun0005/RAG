...
class VectorStoreManager:
    ...
    def _close_client(self, store_id: str):
        ...
        try:
            if store_id in self.client_cache:
                # ChromaDB doesn't have explicit close, but we can clear the cache
        ...
        try:
            ...
            for i in range(0, total_nodes, batch_size):
                ...
                try:
                    ...
                    # Create/update index with batch of nodes
                    VectorStoreIndex(
                        batch,
    ...
    def delete_vector_store(self, store_id: str, store_name: str):
        ...
        try:
            ...
            logger.info(f"🗑️ Deleting vector store: {store_name} (ID: {store_id})")
            logger.info(f"   Collection: {collection_name}")
            logger.info(f"   Path: {store_path}")

            # Step 1: Remove from caches first
            if store_id in self.vector_store_cache:
                del self.vector_store_cache[store_id]
                logger.debug("   ✓ Removed from vector store cache")

            # Step 2: Delete collection from ChromaDB
            try:
                client = self._get_client(store_id, store_name)
            ...
            # Step 3: Close and remove client
            self._close_client(store_id)
            logger.debug("   ✓ Closed ChromaDB client")

            # Step 4: Force garbage collection to release file handles
            import gc
            gc.collect()
            time.sleep(0.5)  # Give OS time to release file handles

            # Step 5: Delete the physical folder
            if os.path.exists(store_path):
                try:
                    # Try multiple times if needed (Windows file locking issues)
                    max_retries = 3
                    for attempt in range(max_retries):
                        try:
                            ...
                            if attempt < max_retries - 1:
                                ...
                                logger.warning(f"   ⚠️ Forced deletion with ignore_errors")
                except Exception as e:
                ...
                logger.warning(f"   ⚠️ Folder does not exist: {store_path}")
