...
class ChatbotUI {
    ...
    async handleSendMessage(event) {
        ...
        try {
            ...
            console.log('chunks is array?:', Array.isArray(data.chunks));
            console.log('chunks length:', data.chunks ? data.chunks.length : 'null/undefined');
            if (data.chunks && data.chunks.length > 0) {
                console.log('First chunk:', data.chunks[0]);
    ...
    addMessage(role, content, chunks = []) {
    ...
    formatSourceDocuments(chunks) {
        ...
        console.log('DEBUG: First chunk structure:', chunks[0]);
        console.log('DEBUG: First chunk keys:', Object.keys(chunks[0]));
        // DEBUG LOGGING END
        ...
        return `
            <div class="source-documents">
                ...
                ${chunks.map((chunk, index) => {
                    // DEBUG: Log each chunk individually
                    console.log(`DEBUG: Processing chunk ${index + 1}/${chunks.length}`);
                    console.log(`DEBUG: Chunk ${index + 1} raw data:`, chunk);
                    console.log(`DEBUG: Chunk ${index + 1} keys:`, Object.keys(chunk));

                    // Try multiple property access patterns with logging
                    console.log(`DEBUG: Chunk ${index + 1} file_name:`, chunk.file_name);
                    console.log(`DEBUG: Chunk ${index + 1} source:`, chunk.source);
                    console.log(`DEBUG: Chunk ${index + 1} metadata:`, chunk.metadata);
                    console.log(`DEBUG: Chunk ${index + 1} text:`, chunk.text ? `${chunk.text.substring(0, 50)}...` : 'undefined');
                    console.log(`DEBUG: Chunk ${index + 1} content:`, chunk.content ? `${chunk.content.substring(0, 50)}...` : 'undefined');
                    console.log(`DEBUG: Chunk ${index + 1} score:`, chunk.score);
                    console.log(`DEBUG: Chunk ${index + 1} page:`, chunk.page);

                    // Extract data from chunk object with multiple fallbacks
