...
# Add the parent directory to Python path to access ingestion module
...
class Retriever:
    def __init__(
        ...
        self.top_k = top_k
        # Convert threshold to cosine similarity range if needed (-1 to 1)
    ...
    def _normalize_threshold(self, threshold: float) -> float:
        ...
        """
        # If already in proper range, return as is
        if threshold < 0:
            return threshold

        # Convert 0-1 range to -1 to 1 range
        # threshold 0.2 becomes -0.6 (very permissive)
        # threshold 0.5 becomes 0.0 (moderate)
        # threshold 0.7 becomes 0.4 (strict)
    ...
    async def retrieve_context(self, query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Step 1: Get vector store index using LlamaIndex
            ...
            # Step 2: Increase top_k to get more candidates before filtering
            ...
            if not formatted_results:
                logger.warning(f"⚠️ No documents passed score threshold ({self.score_threshold:.3f})")
            ...
            # Step 5: Limit to requested top_k
            formatted_results = formatted_results[:self.top_k]

            # Step 6: Build context and collect sources
            ...
            # ✅ DEBUG: Log the final chunks being returned
            ...
            # Update LangFuse with retrieval results
            if LANGFUSE_ENABLED:
        ...
        for idx, node in enumerate(nodes):
            if hasattr(node, 'node') and node.node:
                ...
                if score >= self.score_threshold:
                    # Extract file_name with multiple fallbacks
    ...
    def _build_context(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        """
        # Step 1: Build context text by combining chunks
        context_chunks = []
        ...
        # Step 2: Collect source information if requested
        sources = []
        chunks_data = []
        ...
        if self.include_source:
            ...
            # Prepare detailed chunks data with ALL required fields
            for i, result in enumerate(results):
                chunk_obj = {
                ...
                if not chunk_obj["text"] or not chunk_obj["file_name"]:
                    logger.warning(f"⚠️ Chunk {i + 1} missing required fields, skipping")
                    continue
